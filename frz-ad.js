(function(){
'use strict';
var BASE_URL='https://farazahammed.github.io/FRZ-AD-SERVER/frzads/';
var ADS=[
    { img: 'frzgames.png.png', link: 'https://frz-games.netlify.app', label: 'frz games' },
    { img: 'eduport.png.png', link: 'https://sslc-notes-frz.blogspot.com', label: 'frz notes' },
    { img: 'eid.png.png', link: 'https://support-frz.netlify.app', label: 'eid' }
];
...
function buildOverlay(){if(document.getElementById(AD_ID))return;injectStyles();var ov=document.createElement('div');ov.id=AD_ID;var card=document.createElement('div');card.id='frz-ad-card';var imgWrap=document.createElement('div');imgWrap.id='frz-ad-img-wrap';var imgEl=document.createElement('img');imgEl.id='frz-ad-img';imgEl.alt='Advertisement';imgEl.src=BASE_URL+ADS[0].img;imgEl.style.cursor='pointer';imgEl.onclick=function(){goToLink();};imgWrap.appendChild(imgEl);var label=document.createElement('div');label.id='frz-ad-label';label.textContent=ADS[0].label||'';imgWrap.appendChild(label);var dotsWrap=document.createElement('div');dotsWrap.id='frz-ad-dots';ADS.forEach(function(_,i){var dot=document.createElement('button');dot.className='frz-dot'+(i===0?' frz-active':'');dot.setAttribute('aria-label','Slide '+(i+1));dot.onclick=function(){goTo(i);};dotsWrap.appendChild(dot);});imgWrap.appendChild(dotsWrap);if(ADS.length>1){var prev=document.createElement('button');prev.className='frz-arrow';prev.id='frz-ad-prev';prev.innerHTML='&#8249;';prev.onclick=function(){goTo((currentSlide-1+ADS.length)%ADS.length);};imgWrap.appendChild(prev);var next=document.createElement('button');next.className='frz-arrow';next.id='frz-ad-next';next.innerHTML='&#8250;';next.onclick=function(){goTo((currentSlide+1)%ADS.length);};imgWrap.appendChild(next);}var closeBtn=document.createElement('button');closeBtn.id='frz-ad-close';closeBtn.innerHTML='&#x2715;';closeBtn.setAttribute('aria-label','Close ad');closeBtn.onclick=function(){hideAd();};var progress=document.createElement('div');progress.id='frz-ad-progress';var progressBar=document.createElement('div');progressBar.id='frz-ad-progress-bar';progress.appendChild(progressBar);var brand=document.createElement('div');brand.id='frz-ad-brand';brand.innerHTML='<span class="frz-b1">Ad by <b>FRZ-AD</b> Service</span><span class="frz-b2">ad powered by frz-ad service</span>';card.appendChild(closeBtn);card.appendChild(imgWrap);card.appendChild(progress);card.appendChild(brand);ov.appendChild(card);ov.addEventListener('click',function(e){if(e.target===ov)hideAd();});document.body.appendChild(ov);injected=true;}
function goTo(index){if(ADS.length<2)return;clearInterval(slideTimer);var imgEl=document.getElementById('frz-ad-img');var label=document.getElementById('frz-ad-label');var dots=document.querySelectorAll('.frz-dot');imgEl.classList.add('frz-fade');setTimeout(function(){currentSlide=index;imgEl.src=BASE_URL+ADS[currentSlide].img;if(label)label.textContent=ADS[currentSlide].label||'';imgEl.classList.remove('frz-fade');dots.forEach(function(d,i){d.className='frz-dot'+(i===currentSlide?' frz-active':'');});},260);startSlideTimer();startProgress();}
...
function showAd(){if(!injected)buildOverlay();var ov=document.getElementById(AD_ID);if(!ov)return;currentSlide=0;var imgEl=document.getElementById('frz-ad-img');if(imgEl){imgEl.src=BASE_URL+ADS[0].img;imgEl.classList.remove('frz-fade');}var label=document.getElementById('frz-ad-label');if(label)label.textContent=ADS[0].label||'';var dots=document.querySelectorAll('.frz-dot');dots.forEach(function(d,i){d.className='frz-dot'+(i===0?' frz-active':'');});requestAnimationFrame(function(){ov.classList.add('frz-show');});if(ADS.length>1){startSlideTimer();startProgress();}}
function hideAd(){var ov=document.getElementById(AD_ID);if(ov)ov.classList.remove('frz-show');clearInterval(slideTimer);var bar=document.getElementById('frz-ad-progress-bar');if(bar){bar.style.transition='none';bar.style.width='0%';}resetInactivityTimer();}
